---
description: viberank(viberank.app) 연동을 켜고 끈다. 켜면 /claude-run 한 번에 clauderank·viberank 양쪽이 갱신되고, 내 viberank 순위가 리포트에 함께 표시된다. Usage - /claude-run-viberank [on|off|status] [깃헙유저명]. "viberank 연동", "바이브랭크", "둘 다 올려줘", "다른 리더보드도" 요청 시 사용.
...
viberank 는 절대 금액 누적 랭킹이고 clauderank 는 구독료 대비 배율이라, 축이 달라 같이 보면 서로를 보완한다.
...
- **on** — 연동을 켠다. GitHub 유저명은 `gh api user` 로 자동 판별하며, 인자로 직접 줄 수도 있다.
  ...
  - 내 **viberank 순위**를 읽어 clauderank 리포트 상단에 뱃지로 표시한다.
...
- 순위 조회는 **이 기기에서** 한다. clauderank 서버가 viberank 를 대신 긁지 않는다.
- viberank 쪽이 느리거나 실패해도 **clauderank 제출은 그대로 진행**된다.
